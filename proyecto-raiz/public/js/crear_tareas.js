...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log("--- INICIANDO SCRIPT DE CREAR TAREAS ---");
    console.log("Conectando a API en:", RUTA_API);
    ...
    async function init() {
        ...
        selectGrupo.addEventListener('change', function() {
            ...
            // Protección: si no hay dataset, no hacemos nada
            if (!opcion || !opcion.dataset) return;

            const idGrupo = opcion.dataset.idGrupo;
            console.log("Grupo cambiado. ID seleccionado:", idGrupo);
    ...
    async function cargarGruposAdmin() {
        try {
            console.log("Solicitando grupos a la API...");
            const res = await fetch(`${RUTA_API}?controlador=Grupo&accion=misGrupos`);
            const grupos = await res.json();
            
            console.log("Respuesta API recibida:", grupos); // AQUÍ VERÁS EL JSON EN CONSOLA
            ...
            grupos.forEach(g => {
                ...
                console.log(`Analizando Grupo: ${g.nombre}`);
                console.log(`   > Rol: "${rol}" | Es Admin/Editor? ${esAdmin}`);
                console.log(`   > Calendario ID: ${g.id_calendario} | Tiene Calendario? ${tieneCalendario}`);
            ...
            if (gruposValidos === 0) {
                ...
                // Habilitamos el select por si estaba deshabilitado
                selectGrupo.disabled = false;
            }

        } catch (error) {
            console.error("Error FATAL cargando grupos:", error);
